...
class WesternPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `western`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM western WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO western (date, title, link, content)"
